...
import { NutritionistIcon, QrCodeIcon } from './Icons'; // Using the new icon
...
export const Nutritionist: React.FC<NutritionistProps> = ({
    ...
    return (
        <div className="flex flex-col h-full bg-gradient-to-br from-green-500 to-teal-800 text-white relative">
            <header className="bg-black/20 backdrop-blur-sm shadow-sm p-3 flex items-center justify-between sticky top-0 z-10 border-b border-white/10">
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        <h1 className="text-xl md:text-2xl font-bold">Async+ Nutricionista</h1>
                        <p className="text-xs text-white/70">Guia para uma alimentação saudável</p>
                    </div>
                </div>
                ...
                <div className="max-w-3xl mx-auto w-full space-y-6">
                    {messages.length === 0 && !isLoading && (
                        <div className="text-center py-10">
                            ...
                            <h2 className="text-xl font-semibold">Bem-vindo(a) ao seu espaço de nutrição!</h2>
                            <p className="text-white/80 mt-2">Peça um plano alimentar, analise uma receita ou tire dúvidas sobre alimentos.</p>
                        </div>
                    )}
                    {messages.map((msg) => (
